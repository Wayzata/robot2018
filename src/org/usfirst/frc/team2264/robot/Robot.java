...
// Main class that will use other classes and call methods to run the robot
	...
	*/
	 ...
	 */
		...
		case driveAuto:
			auto.driveForward(frontLeft, frontRight, backLeft, backRight, Gyro, 0);
			break;
		default:
	...
	/* Cameron's shooterboi, grabberboi, movyboi classes
	void checkButtons() {
		if(controller.getAButtonPressed()) {
			shooter.startShooter(shooterLeft, shooterRight);
		}
		if(controller.getBButtonPressed()) {
			shooter.stopShooter(shooterLeft, shooterRight);
		}
		if(controller.getXButtonPressed()) {
			grabber.startIntake(intakeLeft, intakeRight);
		}
		if(controller.getYButtonPressed()) {
			grabber.stopIntake(intakeLeft, intakeRight);
		}
		if(controller.getBumperPressed(GenericHID.Hand.kLeft)) {
			mover.startConveyor(conveyorLeft, conveyorRight);
		}
		if(controller.getBumperPressed(GenericHID.Hand.kRight)) {
			mover.stopConveyor(conveyorLeft, conveyorRight);
		}
	}
	*/
